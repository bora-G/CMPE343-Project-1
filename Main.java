...
public class Main {
    ...
    /**
     * Helper method to read an integer within a specific range [min, max] with
     * validation.
     */
    private static int readInt(Scanner scan, int min, int max) {
        while (true) {
            ...
            if (s.isEmpty()) {
                System.out.print(RED + "Input cannot be empty. Please enter a number between " + min + " and " + max
                        + ": " + RESET);
                continue;
            }
            if (!s.matches("\\d+")) {
                System.out.print(
                        RED + "Invalid input. Please enter a number between " + min + " and " + max + ": " + RESET);
                continue;
            }
            ...
            try {
                ...
                if (val < min || val > max) {
                    System.out.print(
                            RED + "Number out of range. Please enter between " + min + " and " + max + ": " + RESET);
                    continue;
                }
                return val;
            } catch (NumberFormatException e) {
                System.out.print(RED + "Invalid number format (too large). Please enter between " + min + " and " + max
                        + ": " + RESET);
            }
        }
    }
    ...
    private static void evaluationOfExpression() {
        while (true) {
            ...
            try {
                ...
                // Her ArithmeticException için doğru mesajı göster
...
private static long safeAdd(long a, long b) {
    ...
    catch (ArithmeticException e) { throw overflowEX(); }
}
...
private static long safeDiv(long a, long b) {
    ...
    if (a == Long.MIN_VALUE && b == -1) throw overflowEX(); // only overflow case for long division
    return a / b;
}
    ...
    /**
     * Recursively reduces a mathematical expression step-by-step, following
     * operator precedence.
     */
    ...
    /**
     * Applies the operator at index `opIndex` to the numbers immediately left and
     * right.
     */
   ...
   private static String exprApplyOp(String s, int idx) {
    ...
    /**
     * Returns the harmonic mean of {@code arrC}; returns {@code NaN} if any value
     * is zero.
     ...
     * 
     * @return similarity in [-1,1], or {@code null} if either vector has zero
     *         magnitude
     */
