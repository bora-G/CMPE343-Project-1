...
public class Main {
    ...
    // Limits for Option B
    private static final int B_MIN_N = 12;
    private static final int B_MAX_N = 10_000_000;
    ...
    /**
     * Clears the screen and renders the project welcome banner (ASCII art),
     * team member section, and a prompt to continue.
     * Waits for the user to press ENTER before returning.
     *
     * Side effects:
     * - Uses {@code clearScreen()} to reset the console.
     * - Prints multi-line colored ASCII banners via ANSI escape codes.
     *
     * @author CanErsan
     * @author bora-G
     * @author Mikail Karacaer
     * @author Melek Sadiki
     */
    ...
    /**
     * Displays the main menu and routes to the selected section:
     * <ul>
     * <li><strong>A</strong> – Primary School (subMenuOption1)</li>
     * <li><strong>B</strong> – Secondary School (subMenuOption2)</li>
     * <li><strong>C</strong> – High School (subMenuOption3)</li>
     * <li><strong>D</strong> – University (subMenuOption4)</li>
     * <li><strong>E</strong> – Terminate</li>
     * </ul>
     *
     * The loop continues until the user selects <em>Terminate</em>.
     * Input is validated via {@code readMenuOption(SC, 'A', 'E')}.
     *
     * Side effects:
     * - Clears the screen between interactions.
     * - Invokes submenu handlers based on user choice.
     *
     * @see #subMenuOption1()
     * @see #subMenuOption2()
     * @see #subMenuOption3()
    ...
    /**
     * Reads a single uppercase menu letter within the inclusive range [min..max].
     *
     * @param scan input scanner
     * @param min  minimum acceptable option (inclusive)
     * @param max  maximum acceptable option (inclusive)
     * @return the validated uppercase option character
     */
    ...
    private static void clearScreen() {
        System.out.print("\033[H\033[2J\033[3J"); // move cursor home, clear screen, clear scrollback
        System.out.flush();
    }
    ...
    /**
     * Displays the Primary School submenu and routes to the selected task:
     * <ul>
     * <li><strong>A</strong> – Age and Zodiac Sign Calculation</li>
     * <li><strong>B</strong> – Reverse the Words (Recursive)</li>
     * <li><strong>C</strong> – Return to Main Menu</li>
     * </ul>
     *
     * The method continuously listens for user input using {@code readMenuOption}
     * and executes the appropriate task based on user selection.
     *
     * @author CanErsan
     */
    ...
    /**
     * Calculates the user's age (years/months/days)
     * and zodiac sign based on the birth date.
     * Handles invalid inputs and prevents future or zero-padded dates.
     *
     * Logic:
     * - Requests year, month, and day from the user
     * - Calculates age difference using current date
     * - Displays zodiac sign
     */
    ...
    private static void ageAndZodiacSignDetection() {
        while (true) {
            ...
            System.out.print(YELLOW + "Please enter your year of birth (Q to go back): " + RESET);
            int birthYear = getYear(SC);
            if (birthYear == -1)
                return;

            System.out.print(YELLOW + "Please enter your month of birth (1-12, Q to go back): " + RESET);
            int birthMonth = getMonth(SC);
            if (birthMonth == -1)
                return;

            System.out.print(YELLOW + "Please enter your day of birth (Q to go back): " + RESET);
            ...
            System.out
                    .println(CYAN + "Your age: " + years + " years, " + months + " months, " + days + " days" + RESET);
    ...
    /**
     * Returns zodiac sign name based on birth month and day.
     *
     * @param day   birth day
    ...
    /**
     * Returns number of days in a given month considering leap year.
     *
     * @param month month (1-12)
     * @param year  year (for leap-year check)
    ...
    /**
     * Reads and validates DAY input for the given month/year.
     *
     * Rules:
     * <ul>
     * <li>No leading zeros</li>
     * <li>Digits only</li>
     * <li>Must be in the valid range for the month</li>
     * </ul>
     *
     * @param scan  input scanner
     * @param month month (1–12)
     * @param year  year
     * @return valid day, or -1 if user enters 'Q'
     */
    ...
    private static int getDay(Scanner scan, int month, int year) {
        ...
        while (true) {
            ...
            if (s.length() > 1 && s.charAt(0) == '0') {
                System.out.print(RED + "Leading zeros are not allowed. Enter day (1-" + maxDay + "): " + RESET);
                continue;
            }
            if (!s.matches("\\d+")) {
                System.out.print(RED + "Digits only. Enter day (1-" + maxDay + "): " + RESET);
                continue;
            }
            try {
                int val = Integer.parseInt(s);
                if (val < 1 || val > maxDay) {
                    System.out.print(RED + "Day must be between 1 and " + maxDay + ": " + RESET);
                    continue;
                }
                return val;
    ...
    /**
     * Reads and validates MONTH input (1–12).
     *
     * @param scan input scanner
     * @return valid month, or -1 if user enters 'Q'
     */
    ...
    private static int getMonth(Scanner scan) {
        while (true) {
            ...
            if (s.equalsIgnoreCase("Q"))
                return -1;
            if (s.length() > 1 && s.charAt(0) == '0') {
                System.out.print(RED + "Leading zeros are not allowed. Enter month (1-12): " + RESET);
                continue;
            }
            if (!s.matches("\\d+")) {
                System.out.print(RED + "Digits only. Enter month (1-12): " + RESET);
                continue;
            }
            try {
                int val = Integer.parseInt(s);
                if (val < 1 || val > 12) {
                    System.out.print(RED + "Month must be between 1 and 12: " + RESET);
                    continue;
                }
                return val;
            } catch (NumberFormatException e) {
     ...
     * <li>No leading zeros</li>
     * <li>No negatives</li>
     * <li>Must be &lt;= current year</li>
     * </ul>
     *
     * @param scan input scanner
     * @return valid year, or -1 if user enters 'Q'
     */
    ...
    private static int getYear(Scanner scan) {
        ...
        while (true) {
            ...
            if (s.equalsIgnoreCase("Q"))
                return -1;
            if (s.length() > 1 && s.charAt(0) == '0') {
                System.out.print(RED + "Leading zeros are not allowed. Enter year (<= " + currentYear + "): " + RESET);
                continue;
            }
            if (!s.matches("\\d+")) {
                System.out.print(RED + "Digits only. Enter year (<= " + currentYear + "): " + RESET);
                continue;
            }
            try {
                int val = Integer.parseInt(s);
                if (val < 0) {
                    System.out.print(RED + "Year cannot be negative. Enter year: " + RESET);
                    continue;
                }
                if (val > currentYear) {
                    System.out.print(RED + "Year cannot be in the future (<= " + currentYear + "): " + RESET);
                    continue;
                }
                return val;
            } catch (NumberFormatException e) {
    ...
    /**
     * Returns current day from system clock.
     * 
     * @return day of month
     */
    ...
    /**
     * Returns current month from system clock.
     * 
    ...
    /** @return current year from system clock */
    private static int getCurrentYear() {
        return LocalDate.now().getYear();
    }

    /**
     * Displays Reverse Words screen and processes user text.
     * Uses recursive reverseWordRec to reverse only words with >= 2 characters.
     *
     * Stops if user enters Q.
     */
    ...
    private static void reverseTheWords() {
        while (true) {
            ...
            if (input.equalsIgnoreCase("Q")) {
                System.out.println(YELLOW + "Returning to previous menu..." + RESET);
                return;
            }
    ...
    /**
     * Reads a full line of text (Turkish characters allowed).
     *
     * @param scan input scanner
     * @return the entered line, or "Q" to go back
     */

    private static String getTextInput(Scanner scan) {
        System.out.print(GREEN + "Enter text (Turkish characters allowed, Q to go back): " + RESET);
        while (true) {
    ...
    /**
     * Splits the string into a sequence of:
     * - word blocks (letters or apostrophes)
     * - non-word blocks (digits, punctuation, space)
     *
     * Words with >= 2 characters are reversed using recursion.
     *
     * @param s original user input
     * @return processed string with reversed words
     */
    ...
    /**
     * Validates that input has at least 2 meaningful characters.
     *
     * @param s user input
     * @return true if reverse operation is allowed
     */
    ...
    /**
     * Recursive function to reverse a word (Task requirement).
     *
     * @param s word to reverse
    ...
    /**
     * Reads a number between 12 - 10,000,000 and computes all primes up to n using
    ...
    /**
     * Reads a prime-number upper bound from user input (Option B).
     * <p>
     * Rules:
     * <ul>
     * <li>Accepts only integer input in the range [B_MIN_N, B_MAX_N]</li>
     * <li>User may type <strong>Q</strong> (case-insensitive) to go back</li>
     * </ul>
     *
     * @return the validated upper limit as Integer, or {@code null} if user enters
     *         'Q'
     */
    private static Integer getPrimeNumberInput() {
        while (true) {
            System.out.println(GREEN + "Enter the upper limit for prime search or Q to go back:" + RESET);
            System.out.println(YELLOW + "Minimum allowed: " + B_MIN_N + RESET);
            System.out.println(YELLOW + "Maximum allowed: " + B_MAX_N + RESET);
            ...
            try {
                ...
                if (n >= B_MIN_N && n <= B_MAX_N) {
                    return n;
                }
            } catch (Exception ignored) {
            }

            System.out.println(RED + "Invalid input. Please enter a number between " + B_MIN_N + " and " + B_MAX_N
                    + ", or Q to go back." + RESET);
        }
    }
   ...
   private static int[] sieveSundaram(int n) {
    ...
    /**
     * Reads an arithmetic expression (digits, +, -, ×, : , parentheses),
    ...
    private static void evaluationOfExpression() {
        while (true) {
            ...
            System.out.print(GREEN + "Enter expression using digits, +, -, x, :, (, ) or Q to go back: " + RESET);
            ...
            while (true) {
                ...
                if (!exprIsValid(expr)) {
                    ...
                    System.out.print(GREEN + "Enter expression or Q to go back: " + RESET);
                    continue;
                }
    ...
    /**
     * Creates a unified exception instance used when any arithmetic
     * operation results in overflow or underflow.
     *
     * @return {@code ArithmeticException} with message "Overflow/Underflow"
     */
    private static ArithmeticException overflowEX() {
        return new ArithmeticException("Overflow/Underflow");
    }

    /**
     * Parses a long value from the given string.
     * <p>
     * If the literal is outside the valid {@code long} range, overflow is treated
     * consistently with arithmetic operations by rethrowing {@link #overflowEX()}.
     *
     * @param s input string containing a numeric literal
     * @return parsed long value
     * @throws ArithmeticException if the literal is out of {@code long} range
     */
    private static long parseStrictLong(String s) {
        try {
            return Long.parseLong(s);
        } catch (NumberFormatException e) {
            throw overflowEX(); // literal out of range
        }
    }

    /**
     * Performs safe addition of two {@code long} values.
     * Internally uses {@link Math#addExact(long, long)} to detect overflow.
     *
     * @param a first operand
     * @param b second operand
     * @return result of {@code a + b}
    ...
    /**
     * Performs safe subtraction of two {@code long} values.
     * Uses {@link Math#subtractExact(long, long)} to detect overflow.
     *
     * @param a minuend
     * @param b subtrahend
     * @return result of {@code a - b}
    ...
    /**
     * Performs safe multiplication of two {@code long} values.
     * Uses {@link Math#multiplyExact(long, long)} to detect overflow.
     *
     * @param a first operand
     * @param b second operand
     * @return result of {@code a * b}
    ...
    /**
     * Performs safe division of two {@code long} values.
     * <ul>
     * <li>Throws exception on division by zero</li>
     * <li>Detects and rejects the only overflow case:
     * {@code Long.MIN_VALUE / -1}</li>
     * </ul>
     *
     * @param a dividend
     * @param b divisor
    ...
    private static long safeDiv(long a, long b) {
        ...
        if (a == Long.MIN_VALUE && b == -1)
            throw overflowEX(); // only overflow case for long division
        return a / b;
    }

    /**
     * Validates the structure and characters of a mathematical expression.
     *
     * @param s normalized expression string
    ...
    private static boolean exprIsValid(String s) {
        ...
        // allow only digits, space, +, -, x, :, (, )
        if (!s.matches("[0-9\\s+\\-x:()]+"))
            return false;

        // normalize to internal ops (* and /) only from 'x' and ':'
        ...
        // cannot start with binary op; leading '-' is allowed if followed by digit
        if (isOp(t.charAt(0)) && t.charAt(0) != '-')
            return false;
        if (t.charAt(0) == '-' && (t.length() == 1 || !Character.isDigit(t.charAt(1))))
            return false;

        // cannot end with an operator
        if (isOp(t.charAt(t.length() - 1)))
            return false;

        // reject ")(" adjacency and digit before '(' or ')' before digit
        if (t.contains(")(") || t.matches(".*\\d\\(.*") || t.matches(".*\\)\\d.*"))
            return false;

        // parentheses balance and no empty pairs after normalization
        int balance = 0;
        ...
        // operator adjacency/placement rules incl. unary minus after operator or '('
        for (int i = 1; i < t.length(); i++) {
            char cur = t.charAt(i);
            char prev = t.charAt(i - 1);

            if (cur == '-') {
                // unary minus allowed after operator or '(' if followed by a digit
                if (isOp(prev) || prev == '(') {
                    if (i + 1 >= t.length() || !Character.isDigit(t.charAt(i + 1)))
                        return false;
                    continue;
                }
                // otherwise binary minus: previous must be digit or ')'
                if (isOp(prev) || prev == '(')
                    return false;
                continue;
            }
            ...
            if (cur == '+' || cur == '*' || cur == '/') {
                // binary op must follow a digit or ')'
                if (isOp(prev) || prev == '(')
                    return false;
            }
        }
    ...
    // replace your exprNormalize with this (only 'x' and ':' are accepted as
    // mul/div)
     ...
     */
    // keep using x and : when printing steps
    private static String exprDenorm(String s) {
        return s.replace('*', 'x').replace('/', ':');
    }

    // optional: ensure only '+', '-', '*', '/' are treated as ops internally
    private static boolean isOp(char c) {
        return c == '+' || c == '-' || c == '*' || c == '/';
    }

    /**
     * Normalizes and evaluates an expression, printing the first normalized step.
     *
     * @param expr raw user expression
     */
    ...
    /**
     * Recursively reduces an expression step-by-step following precedence.
     *
     * @param s           normalized expression
     * @param shouldPrint whether to print each reduction step
     * @return the reduced expression as a number string
     */
        ...
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) == '(') {
                ...
                if (currentDepth == maxDepth && deepestOpenParenIndex != -1) {
                    ...
                    if (insideResult.startsWith("-") && !prefix.isEmpty()
    ...
    /**
     * Applies the operator at index {@code idx} to the adjacent operands.
     *
     * @param s   normalized expression
     * @param idx operator index
     * @return expression with that operation applied
     */
    ...
    private static String exprApplyOp(String s, int idx) {
        ...
        if (L >= 0 && s.charAt(L) == '-' && (L == 0 || isOp(s.charAt(L - 1)) || s.charAt(L - 1) == '(')) {
            // include unary minus on the left number
        } else {
            L++;
        }
        ...
        long a = parseStrictLong(left);
        long b = parseStrictLong(right);

        long result;
        char op = s.charAt(idx);

        switch (op) {
            case '*' -> result = safeMul(a, b);
            case '/' -> result = safeDiv(a, b);
            case '+' -> result = safeAdd(a, b);
            case '-' -> result = safeSub(a, b);
            default -> throw new IllegalStateException("Unknown operator: " + op);
        }

        String resultStr = Long.toString(result);
        if (result < 0 && L > 0 && s.charAt(L - 1) == '+') {
    ...
    /**
     * Removes a single pair of <i>outermost matching parentheses</i> from the
     * expression if and only if they wrap the entire string.
     * <p>
     * Example:
     * 
     * <pre>
     * "(1 + (2 * 3))"  ->  "1 + (2 * 3)"
     * "( (1 + 2) )"    ->  "(1 + 2)"
     * "(1 + 2) + 3"    ->  unchanged (parentheses do not wrap entire string)
     * </pre>
     *
     * @param s expression string (already normalized)
     * @return expression without redundant wrapping parentheses
     */
     ...
     * <p>
     * A: stats on a single array, B: distances between two arrays, C: back to main
     * menu.
     * </p>
     */
    ...
    private static ArrayList<Double> readStatsArrayOrQuit(int n) {
        ...
        System.out.println(CYAN + "Enter " + n + " numbers (<= " + (int) C_MAX_VALUE
                + "). Use space/newline. Type Q anytime to quit." + RESET);
        while (list.size() < n) {
            ...
            for (String t : tok) {
                ...
                try {
                    ...
                    if (v > C_MAX_VALUE) {
                        System.out.println(RED + "Value " + t + " exceeds " + (int) C_MAX_VALUE + ". Please enter <= "
                                + (int) C_MAX_VALUE + "." + RESET);
                        lineBad = true;
                        break;
                    }
                    lineVals.add(v);
            ...
            if (lineVals.size() > remaining) {
                System.out.println(
                        RED + "Warning: " + (lineVals.size() - remaining) + " extra value(s) ignored." + RESET);
            }
        }
        return list;
    }
    ...
    private static int[] readVectorCOrQuit(int d, String name) {
        while (true) {
            ...
            int i = 0;
            System.out.println(
                    CYAN + "Enter vector " + name + " (length " + d + ", digits 0-9). Type Q to quit." + RESET);
            ...
            while (i < d) {
                ...
                if (bad) {
                    ...
                    System.out.println(
                            RED + "Warning: " + extras + " extra value(s) ignored for vector " + name + "." + RESET);
                }
            }
            return v;
        }
    ...
    /**
     * Interactive workflow for the "Statistical Information" feature (High School
     * menu).
     * <p>
     * Steps performed in a loop:
     * <ol>
     * <li>Prompts user for array size (user may type {@code Q} to return)</li>
     * <li>Prompts user to enter the elements of the array</li>
     * <li>Stores the values in the shared container {@code arrC}</li>
     * <li>Computes:
     * <ul>
     * <li>Median</li>
     * <li>Arithmetic mean</li>
     * <li>Geometric mean (may be undefined if a value ≤ 0)</li>
     * <li>Harmonic mean (may be undefined if a value is 0)</li>
     * </ul>
     * </li>
     * <li>Displays the results formatted to 6 decimal places</li>
     * </ol>
     * The loop continues until the user selects {@code N} when asked to process
     * another array.
     */
    ...
    private static void statisticalInformation() {
        while (true) {
            ...
            if (Double.isNaN(geometric))
                System.out.println(
                        CYAN + "Geometric mean:   " + RESET + RED + "undefined (non-positive element)" + RESET);
            else
                System.out.printf(CYAN + "Geometric mean:   " + RESET + "%.6f%n", geometric);
            if (Double.isNaN(harmonic))
                System.out.println(CYAN + "Harmonic mean:    " + RESET + RED + "undefined (element is 0)" + RESET);
            else
    ...
    /**
     * Returns the median of {@code arrC}. Uses a sorted copy; supports even/odd
     * lengths.
     * 
    ...
    /**
     * Returns the geometric mean of {@code arrC} if all values are positive;
     * otherwise {@code NaN}.
     * 
    ...
    /**
     * Returns the harmonic mean of the current array (Option C).
     *
     * @return harmonic mean value, or {@code NaN} if undefined
     */
    ...
    private static void distanceBetweenTwoArrays() {
        while (true) {
            ...
            if (cosine == null)
                System.out.println(
                        CYAN + "Cosine similarity:  " + RESET + RED + "undefined (one vector is zero)" + RESET);
            else
    ...
    /**
     * Computes the Manhattan distance between two equal-length vectors.
     *
     ...
     * @param a first vector
     * @param b second vector
     * @return cosine similarity in [-1, 1], or {@code null} if a vector is zero
     */
     ...
     *
     * <p>
     * If the user selects "Return to Main Menu", the method returns immediately.
     * </p>
     *
     * @author bora-G
     */
     ...
     *
     * <p>
     * The game will continue until one of these conditions occurs:
     * </p>
     * <ul>
     ...
     *
     * <p>
     * If single-player mode is chosen, the CPU plays using {@link #makeMoveCPU()}.
     * Otherwise, both players use {@link #makeMovePlayer()}.
     * </p>
     *
     * @author bora-G
     */
     ...
     *
     * <p>
     * If the current player is 'X', switches to 'O', and vice-versa.
     * </p>
     */
     ...
     *
     * <p>
     * ANSI escape codes are used for visual coloring.
     * </p>
     */
     ...
     *
     * <p>
     * Prompts the player to choose a column (1..COLS). If the input is invalid or
     * the column is full, the user is prompted again.
     * </p>
     *
     * <p>
     * If the user enters 'Q' or 'q', the method treats it as a forfeit and returns
     * false.
     * </p>
     *
    ...
    private static boolean makeMovePlayer() {
        while (true) {
            System.out.print(YELLOW + "Player " + currentPlayer + ", choose a column (1-" + COLS
                    + ") or enter 'Q' to forfeit: " + RESET);
     ...
     * This is a deterministic rule-based algorithm. No ML/prediction is used.
     * </p>
     */
     ...
     * @return the row index where a token can be placed, or -1 if the column is
     *         full
     */
    ...
    /**
     * Checks whether the last placed token at (row, column) creates a winning
     * condition
    ...
    /**
     * Scans the current board for a connect-four for the current player.
     *
     * @return true if the current player has a winning line
     */
    ...
    private static boolean checkWin() {
        ...
        for (int r = 0; r <= ROWS - 4; r++) {
            for (int c = 0; c <= COLS - 4; c++) {
                if (board[r][c] == p && board[r + 1][c + 1] == p && board[r + 2][c + 2] == p
                        && board[r + 3][c + 3] == p)
                    return true;
            }
        }
     ...
     * Program entry point.
     * <p>
     * Initializes the application by displaying the welcome screen,
     * showing the main menu, and closing the shared {@code Scanner}
     * instance on exit.
     *
